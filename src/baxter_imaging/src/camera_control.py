...
class BaxterImage:
	...
	TURN_OFF = HEAD_CAMERA_NAME
	USE_CAMERA = LEFT_CAMERA_NAME
	...
	def __init__(self, name):
		try:
			print("Closing {}".format(BaxterImage.TURN_OFF))
			CameraController(BaxterImage.TURN_OFF).close()
		except:
			pass
		try:
			...
			self.background.setShadowThreshold(.01)
			self.pub = rospy.Publisher("circ_pos", Circ)
	...
	def imgCallback(self, data):
		...
		if self.calibrate < 150:
			if self.calibrate % 10 == 0:
				print(self.calibrate / 150.0)
			self.background.apply(cvImage)
			self.calibrate += 1
			return
		fgmask = self.background.apply(cvImage, learningRate = 0.005)
		blurred = cv2.medianBlur(fgmask, 11)
		circles = cv2.HoughCircles(blurred, cv2.HOUGH_GRADIENT, 1, 500, param1 = 50, param2 = 25, minRadius = 0, maxRadius = 0)
		# print(circles)
		if circles is not None:
			for i, circle in enumerate(circles[0,:]):
				...
				radius = circle[2]
				# cv2.circle(fgmask, center, 1, (0, 100, 100), 3)
				# cv2.circle(fgmask, center, radius, (255, 0, 255), 3)
				self.pub.publish(Circ(center[0], center[1], radius))
		cv2.imshow('imgShow', fgmask)
		cv2.waitKey(1)
		return
...
if __name__ == "__main__":
	camera = BaxterImage(BaxterImage.USE_CAMERA)
